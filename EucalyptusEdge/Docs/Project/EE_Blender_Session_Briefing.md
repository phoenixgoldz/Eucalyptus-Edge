...
## ===== RIPPER: GOLD-STANDARD ANIMATION PASS COMPLETE (2026-07-17 evening) =====
Torque Rush + Devil's Breaker FROZEN (untouched motion; lossless quat-sign repair only). Old spin archived (_ARCHIVE_Ripper_Heavy_Spin_v1). **RIG FIX: root bone was deforming w/ 2,443 verts (would smear on RM extraction in UE) — redistributed+renormalized, root non-deform now; SK_Ripper.fbx RE-EXPORTED (UE must reimport).** 362 quat sign-flips repaired across all actions (now 0). Walk/Run = IK-contact gait, linear backslide (walk 1.0 m/s, run 2.8 m/s — sync MaxWalkSpeed). **40 takes exported** (full locomotion incl starts/stops/turns, block set+parry, 3 hit reactions, knockdown+2 getups, 4 dodges RM Wren-standard, L3+M1+M2 filling the combat chain, 10 presentation anims). Combo audit: Swipe2->3 and RisingRake->TorqueRush need 8f blends, everything else stance-contract 2f. Full report: RipperModel\RIPPER_ANIMATION_PRODUCTION_REPORT.md (incl. notify windows, blendspace speeds, gold-standard benchmark list for Wren). GAPS: 9 facial keys not on retopo mesh yet (biggest gap); TorqueRush_RM.fbx on disk predates quat repair (no RM action in blend — rebuild root keys from _IP if wanted); Dodge_Fwd awaiting design confirmation; no separate sprint (intentional). EXPORT GOTCHA learned: hidden armature + use_selection silently exports 4KB stubs — size-check every take (>20KB).


## ===== RIPPER: TORQUE RUSH RM REPAIRED + UE ACCEPTANCE GATE OPEN (2026-07-17 night) =====
Phoenix accepted the Blender pass as gold-standard CANDIDATE — NOT frozen until UE acceptance passes. TorqueRush RM recovered from stale FBX into the blend (source of truth now in RipperRetopo.blend), verified bit-identical: 42f, [0,-2.05,0] m root, 0 z-drift, 0 root yaw, 2.05 rotations, 0 quat flips; stale archived _PreQuatFix; corrected authoritative RM re-exported (193KB size-verified). Dodge_Fwd pulled (unapproved) -> _ARCHIVE; blend action _HOLD. Sprint ruled out permanently absent a design change; facial keys deferred to presentation polish. Production folder audited clean (40 active takes + SK + archives). **UE lane: RipperModel\RIPPER_UE_ACCEPTANCE_WORKORDER.md is the authoritative queue** (mesh-first reimport order, root-smear checks, 100/280 cm/s locomotion baselines, 0.267s montage blends to validate, Devil's Breaker two-actor capture spec F20-22 sync / F62-64 release, vs-Wren-victim requirement, 18-point report, PASS/FAIL gate). NO new Ripper polish, no Wren work, until the gate closes.

## ===== RIPPER FOLDER REORGANIZED (2026-07-17) =====
All 40 active animation takes moved to **Desktop\RipperModel\Anims\** (one folder, flat). Root keeps SK_Ripper.fbx, RipperRetopo.blend, Textures\, reports/work order. All archived FBX consolidated into _archive\. UE work-order paths updated accordingly.


## ===== WREN: BENCHMARK ANIMATION PASS COMPLETE (2026-07-17 late) =====
Phoenix directed Wren start (UE Ripper gate runs in parallel on Claude Code's side). Audit: facial keys intact, root clean (flag set non-deform), 88 quat flips repaired (now 0), pre-v2 dodges + import junk archived. **4 repaired _v2 takes validated** (heavy in-place 0-flip; dodges exactly ±0.50/-0.55m) and re-exported under UE-established names. **36 NEW takes authored** on the Ripper benchmark: full locomotion (boxer-shuffle walk 0.85 m/s, KANGAROO BOUND run 2.79 m/s — both-feet gait w/ tail counterweight via TailPlant_IK), block set + parry-slip, hit reactions, knockdown + GetUp_Front + signature GetUp_TailAssist, WO4 ring-out set (Launch/Falling/Land_Hard), boxing chain (Jab/Cross/Hook/BodyStraight/SpringKnee), 10 presentation takes (glove-thud intro/taunt identity). Stance contract: guard+poles derived from shipped idle (0.2 deg reproduction); all boundaries within 8.5 deg -> 2-4f blends. **All 41 takes in KangarooModel\Anims\ (one folder), old loose FBX archived.** Rig doctrine encoded: Y-up data, cosmetic bone tails -> anatomical IK, tail via TailPlant_IK. Report: WREN_ANIMATION_PRODUCTION_REPORT.md. GAPS: **Wren Edge Ultimate not authored — needs Phoenix design ruling (only chain gap)**; SK re-export still gated on 9-key slider test; UE acceptance queued behind Ripper's gate.
