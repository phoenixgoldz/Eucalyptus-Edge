...
- **Source-art work orders for Claude Desktop** (Wren shape-key re-export, Wren/Ripper idle takes, template-gap anims) written to `CLAUDE_DESKTOP_HANDOFF.md`.

**2026-07-17 addendum (session B):** Main Menu music wired (`WAV_Menu_Loop`, Path of Adventure, looping via WBP_MainMenu Construct). Audio volume framework **unified on the submix system** (commit `1f84462`) — a parallel-session duplicate SoundClass/SoundMix set at `Framework/Audio/` was deleted; `ApplyAudioSettings` = the PIE-proven MasterSubmixDefault + EE_MusicSubmix + EE_SFXSubmix version; Master/Music/SFX Options sliders are live on Init and APPLY. **ABP_Wren live-verified** (asset + TargetSkeleton + AnimClass wiring all healthy; heavy/dodge visual check still needs a ~10 s human playtest). Docs (PROJECT_STATE, SESSION_LOG, this file) synced.
